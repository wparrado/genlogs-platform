"""SQLModel table definitions for the GenLogs MVP.

Defines CityReference, Carrier, and CarrierRoute which are used by the
...
class CityReference(SQLModel, table=True):
    """Normalized city entries used for autocomplete fallback."""
...
class CarrierRoute(SQLModel, table=True):
    """Carrier routing and ranking rules."""
    ...
    __table_args__ = (
        UniqueConstraint(
            ...
            "destination_city_id",
            "carrier_id",
            name="uix_route_carrier",
        ),
        CheckConstraint(
            "(origin_city_id IS NULL) = (destination_city_id IS NULL)",
            name="ck_both_null_or_both_not_null",
        ),
    )

    id: Optional[uuid.UUID] = Field(default_factory=uuid.uuid4, primary_key=True)
    origin_city_id: Optional[uuid.UUID] = Field(
        default=None,
        sa_column=Column(ForeignKey("city_reference.id"), nullable=True),
    )
    destination_city_id: Optional[uuid.UUID] = Field(
        default=None,
        sa_column=Column(ForeignKey("city_reference.id"), nullable=True),
    )
